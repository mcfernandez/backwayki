...
export class OptionFormComponent implements OnInit, OnDestroy {
	...
	_pathservice: string = "optionlist";
	_entityname: string = "Opción de la lista";
	_pathurl: string = "/management/optionlist";
	...
	ngOnInit() {
		this.permission = Authorization.Permission(localStorage.getItem('infoauth'), 1, "gestion", "listaopciones");
	...
	getItemList(id: string) {
		this._commonService.List(`${ this._pathservice }/${ id }/optionitem/list`).subscribe((result: any[]) => {
	...
	setItemCreate() {
		...
		var data = { picklistid: this.id, code: this.item.code, name: this.item.name, description: this.item.description };

		this._commonService.Create(`${ this._pathservice }/optionitem/create`, data).subscribe((result: any) => {
			if (result) {						
				this.setItemClear();
				this.getItemList(this.id);
	...
	setItemUpdate() {
		...
		this._commonService.Update(`${ this._pathservice }/optionitem/edit`, data).subscribe((result: any) => {
			if (result) {						
				this.setItemClear();
				this.getItemList(this.id);
	...
	setItemDelete(id: string, name: string) {
		let _entityname = 'El item de la lista de opción';
		...
		dialog.afterClosed().subscribe(result => {
			...
			this._commonService.Delete(`${ this._pathservice }/optionitem/${ id }/delete`).subscribe((result: boolean) => {
				if (result) {
					this.getItemList(this.id);
